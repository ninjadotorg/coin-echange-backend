        ...
        if not referrer:
            ...
            if referrer_rule:
                if not referrer_promotion:
                    ...
                    # Don't need do anymore if violating below rules
                    if referrer_promotion.referral_count >= referrer_rule.first_referral_count:
                        return
                ...
                if (first_referral_count + 1 <= referrer_rule.first_referral_count
                ...
                if order.first_purchase:
                    PromotionOrder.objects.create(
                        order=order,
                        user=user,
                        amount=referrer_rule.first_referral_referee_bonus,
                        currency=referrer_rule.currency,
                        status=REFERRAL_STATUS.pending,
                        referrer=False,
